...
def img_callback(data):
    ...
    image = bridge.imgmsg_to_cv2(data, "bgr8")#ros이미지를 opencv2이미지로 변경함
...
def start():
    ...
    image_sub = rospy.Subscriber("/usb_cam/image_raw/",Image,img_callback)#subscriber에 자동차에서 받은 이미지를 opencv이미지로 변경해서 입력함
    ...
    while not rospy.is_shutdown():
        ...
        f_n += 1
        if (time.time() - t_check) > 1:# 1초마다 시간값 갱신
            #print("fps : ", f_n)
            t_check = time.time()
            f_n = 0

        draw_img = image.copy()# image를 draw_img에 복사
        lpos, rpos, go = process_image(draw_img)#복사한 이미지를 색과 선 분류

        center = (lpos + rpos) / 2
        angle = -(Width/2 - center)

        steer_angle = angle * 0.4#회전하는 각 배율
        draw_steer(steer_angle)

        

        drive(angle, 18)# 각도와 18만큼 운행
